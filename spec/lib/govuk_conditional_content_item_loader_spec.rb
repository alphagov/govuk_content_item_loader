...
RSpec.describe GovukConditionalContentItemLoader do
  ...
  describe "#can_load_from_graphql?" do
    ...
    it "returns false if schema_name is not in allowed schemas" do
      allow(content_store_client).to receive(:content_item).with(base_path)
    ...
    it "returns false if no GraphQL allowed schemas and rates are configured" do
      allow(content_store_client).to receive(:content_item).with(base_path)
        .and_return("schema_name" => "not_allowed")
      allow(Rails).to receive(:application).and_return(
        double(config: OpenStruct.new(
          graphql_allowed_schemas: [],
    ...
    it "returns false if GraphQL traffic rates are not configured via GovukGraphqlTrafficRates initializer" do
      allow(content_store_client).to receive(:content_item).with(base_path)
        .and_return("schema_name" => "not_allowed")
      allow(Rails).to receive(:application).and_return(
        double(config: OpenStruct.new(
          graphql_allowed_schemas: nil,
          graphql_traffic_rates: nil,
        )),
      )
